...
from app.services.admin_invites import create_admin_invite, disable_admin_invite, list_admin_invites, update_admin_invite_note
...
class AccountFoundationTests(unittest.TestCase):
    ...
    def test_administrator_invite_listing_never_exposes_hashes(self) -> None:
        invite = create_admin_invite(self.session, "private-2026", "lookup-key", max_uses=3, admin_note="给测试用户")
        self.session.commit()
        listing = list_admin_invites(self.session)
        self.assertEqual(listing[0]["id"], str(invite.id))
        self.assertEqual(listing[0]["admin_note"], "给测试用户")
        self.assertNotIn("lookup_hash", listing[0])
        self.assertNotIn("secret_hash", listing[0])
        self.assertTrue(update_admin_invite_note(self.session, invite.id, "十月内测"))
        self.assertEqual(list_admin_invites(self.session)[0]["admin_note"], "十月内测")
        with self.assertRaises(ValueError):
            update_admin_invite_note(self.session, invite.id, "x" * 121)
