...
2. `news-ingest.service`：按固定间隔运行抓取、标准化、分类、去重和润色；首次设为每 10 分钟一次，具体间隔通过环境变量配置。
...
现有数据源适配器
  ...
  → 规则分类、标签和评分
  → 写入 news_items
  → 对新入池且需要展示的项目执行一次中文润色
  → 写入 news_polishes 与 token 用量记录
```

分类首先使用可审阅的关键词/来源规则，无法确认时归入“科技”或不入池；不在首版引入不可解释的自动分类模型。时政、财经、社会热搜附带 `source_trust` 或核验状态；页面和消息模板据此展示来源/风险提示。既有 DeepSeek 策略继续使用 `deepseek-v4-flash` 作为主模型、`deepseek-v4-pro` 修复失败项目，并保留可靠的中文兜底内容。润色结果按新闻和提示词版本缓存，绝不随每位订阅者重复生成。
